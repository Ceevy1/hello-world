...
def _run_dynamicfusion(x_seq: np.ndarray, x_stat: np.ndarray, y: np.ndarray, week_idx: np.ndarray, student_ids: np.ndarray, out_dir: Path, seed: int, epochs: int) -> tuple[dict[str, float], np.ndarray]:
...
def run_main_comparison(x_seq: np.ndarray, x_stat: np.ndarray, y: np.ndarray, student_ids: np.ndarray, out: Path, seed: int, epochs: int) -> pd.DataFrame:
    rows = []
    for w_name, w in WINDOWS.items():
        ...
        for name, clf in BASELINES.items():
            ...
            print(f"[Baseline] {name:<18} Acc={met['Accuracy']:.4f} AUC={met['AUC']:.4f} F1={met['F1']:.4f}")
        ...
        print(f"[DynamicFusion-Enhanced] Acc={dm['Accuracy']:.4f} AUC={dm['AUC']:.4f} F1={dm['F1']:.4f}")

    df = pd.DataFrame(rows)
    df.to_csv(out / "performance_comparison_full.csv", index=False)
    print("\n[Summary] Full performance table:")
    print(df.pivot_table(index="model", columns="scenario", values=["Accuracy", "AUC", "F1"]))
    return df
...
def run_lomo_ablation(x_seq: np.ndarray, x_stat: np.ndarray, y: np.ndarray, modules: np.ndarray, out: Path, seed: int) -> pd.DataFrame:
    ...
    for mod in np.unique(modules):
        ...
        for name, cls in models.items():
            ...
            print(f"[Ablation] {name:<10} Acc={met['Accuracy']:.4f} AUC={met['AUC']:.4f} F1={met['F1']:.4f}")
...
def main() -> None:
    ...
    print(early_df.pivot_table(index="model", columns="scenario", values=["Accuracy", "AUC", "F1"]))

    lomo_df = run_lomo_ablation(x_seq, x_stat, y, modules, out, args.seed)
    print("\n[LOMO Ablation Summary]")
    if not lomo_df.empty:
        print(lomo_df.groupby("model")[["Accuracy", "AUC", "F1"]].mean().sort_values("AUC", ascending=False))

    run_shap_analysis(x_seq, x_stat, y, out, args.seed)

    # Export one paper-ready latex table
